                                ...
                                @foreach($cart->items as $item)
                                    ...
                                    <span class="count-number float-right">
                                        <form method="post" action="{{route('cart.update')}}">
                                         @csrf
                                        <input class="count-number-input" type="number" min="1" id="cart"
                                               data-type="{{$item['id']}}"
                                               value="{{$item['quantity']}}">
                                        </form>
                                    </span>
{{--                                         <p>   number_format($item['quantity']*$item['price'])</p>--}}
                                            <p class="text-gray mb-0 float-right ml-5  text-muted big price-show"
                                               style="width: 5rem" data-id="{{$item['id']}}">{{$item['quantity']*$item['price']}}</p>
                                            <a class="btn badge-danger btn-sm"
                                               href="{{route('cart.remove',['id'=> $item['id']])}}">x</a>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="bg-white p-3 clearfix border-bottom">
                                <p class="mb-1">Item Total <span
                                        class="float-right text-dark">{{count($cart->items)}}</span></p>

                                <h6 class="font-weight-bold mb-0">TO PAY
                                    <span class="float-right total-price">{{$cart->total_price }}</span>
                                </h6>
                            </div>
                            <div class="p-3">
                                <a class="btn btn-success btn-block btn-lg" href="{{route('cart.checkout')}}">PAY NOW <i
